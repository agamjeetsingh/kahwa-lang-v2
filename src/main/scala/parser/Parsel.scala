...
type ParserFunction[A, Token, Error] =
  Parsel.Input[Token] => SafePointFunction[Token] ?=> (
      Option[A],
      Parsel.Input[Token],
      Iterable[Error]
  )

class Parsel[+A, Token, Error](
    val parserFunc: ParserFunction[A, Token, Error]
) {
  def apply(input: Parsel.Input[Token])(using
      spFunc: SafePointFunction[Token]
  ): (Option[A], Parsel.Input[Token], Iterable[Error]) = parserFunc(input)

  def map[B](f: A => B)(using
      spFunc: SafePointFunction[Token]
  ): Parsel[B, Token, Error] = Parsel((input: Parsel.Input[Token]) =>
    val (res: Option[A], next: Parsel.Input[Token], errors: Iterable[Error]) =
      parserFunc(input)
    (res.map(f), next, errors)
  )

  def map[B](f: (A, Parsel.Input[Token]) => B)(using
      spFunc: SafePointFunction[Token]
  ): Parsel[B, Token, Error] = Parsel((input: Parsel.Input[Token]) =>
    val (res: Option[A], next: Parsel.Input[Token], errors: Iterable[Error]) =
      parserFunc(input)
    (
      res match {
        case Some(value: A) =>
          Some(
            f(value, input)
          ) // TODO - Not 100% on whether it should be input or next
        case None => None
      },
      next,
      errors
  ...
  def flatMap[B](
      f: A => Option[B]
  )(using spFunc: SafePointFunction[Token]): Parsel[B, Token, Error] =
  ...
  def flatMap[B](
      f: (A, Parsel.Input[Token]) => Option[B]
  )(using spFunc: SafePointFunction[Token]): Parsel[B, Token, Error] =
    Parsel((input: Parsel.Input[Token]) => {
      val (res, next, errors) = parserFunc(input)
      res match {
  ...
  def <~[B](that: Parsel[B, Token, Error])(using
      spFunc: SafePointFunction[Token]
  ): Parsel[A, Token, Error] =
    (this ~ that).map(_._1)

  def ~>[B](that: Parsel[B, Token, Error])(using
      spFunc: SafePointFunction[Token]
  ): Parsel[B, Token, Error] =
    (this ~ that).map(_._2)
}
...
object Parsel {
  ...
  type EnforceStructure[T <: Tuple, K, Token, Error] <: Tuple = T match
    ...
    case _ *: t =>
      (K, Parsel[Any, Token, Error]) *: EnforceStructure[t, K, Token, Error]

  type UnionOfParsers[T <: Tuple, Token, Error] = T match
    case EmptyTuple => Nothing
    case (_, Parsel[r, Token, Error]) *: t =>
      r | UnionOfParsers[t, Token, Error]

  def or[K, Token, T <: Tuple, Error](
      discriminator: Parsel.Input[Token] => Option[K],
      args: T,
      keyFoundButNotInArgsList: K => Iterable[Error]
  )(using
    ...
    Parsel((input: Parsel.Input[Token]) => {
      ...
      keyOpt match {
        case Some(key) =>
          ...
          val entries =
            args.toList.asInstanceOf[List[(K, Parsel[Any, Token, Error])]]
          ...
          entries.find(_._1 == key) match {
            case Some((_, parsel)) =>
              ...
              parsel(input).asInstanceOf[
                (
                    Option[UnionOfParsers[T, Token, Error]],
                    Parsel.Input[Token],
                    Iterable[Error]
                )
              ]
  ...
  def or[A, Token, Error](
      parsers: Parsel[A, Token, Error]*
  ): Parsel[A, Token, Error] = {
    Parsel((input: Parsel.Input[Token]) => {
      @scala.annotation.tailrec
      def tryParsers(
          remaining: Seq[Parsel[A, Token, Error]],
          lastErrors: Iterable[Error]
      ): (Option[A], Parsel.Input[Token], Iterable[Error]) = {
  ...
  def list[A, Token, Error](
      parsel: Parsel[A, Token, Error]
  ): Parsel[List[A], Token, Error] = {
    Parsel((input: Parsel.Input[Token]) => {
      @scala.annotation.tailrec
      def loop(
          currentInput: Parsel.Input[Token],
          lastGoodInput: Parsel.Input[Token],
          acc: List[A],
          accErrors: Iterable[Error]
      ): (Option[List[A]], Parsel.Input[Token], Iterable[Error]) = {
              ...
              case None =>
                ...
                nextInput.current match {
                  case Some(_) =>
                    ...
                    val progressInput =
                      if (syncedInput.index <= lastAttemptPos) {
  ...
  def optional[A, Token, Error](
      parsel: Parsel[A, Token, Error]
  ): Parsel[Option[A], Token, Error] = {
    Parsel((input: Parsel.Input[Token]) => {
  ...
  def sepBy[A, Token, Error, B](
      element: Parsel[A, Token, Error],
      separator: Parsel[B, Token, Error]
  ): Parsel[List[A], Token, Error] = {
    Parsel((input: Parsel.Input[Token]) => {
  ...
  def delay[A, Token, Error](
      parser: => Parsel[A, Token, Error]
  ): Parsel[A, Token, Error] = {
  ...
  class Ops[Expr, Token, Error](
      val fixity: Fixity,
      val ops: Parsel[Any, Token, Error]*
  ) extends OpLevel[Expr, Token, Error]

  object Ops {
    def apply[Expr, Token, Error](fixity: Fixity)(
        ops: Parsel[Any, Token, Error]*
    ): Ops[Expr, Token, Error] =
      new Ops(fixity, ops*)
  }
  ...
  def precedence[Expr, Token, Error](
    ...
    Parsel((input: Parsel.Input[Token]) => {
      ...
      def parsePrefix(
          ...
          prefixOps: List[Parsel[Any, Token, Error]]
      ): (Option[Expr], Parsel.Input[Token], Iterable[Error]) = {
        prefixOps match {
          case Nil =>
            atomParser.parserFunc(input)
          case ops =>
            val prefixParser =
              or(ops.map(_.asInstanceOf[Parsel[Expr => Expr, Token, Error]])*)
            val (opRes, opNext, opErrs) = prefixParser.parserFunc(input)

            opRes match {
              case Some(f) =>
                val (exprRes, exprNext, exprErrs) =
                  parseExprWithLevel(opNext, 0)
      ...
      def parseExprWithLevel(
          input: Parsel.Input[Token],
          minLevel: Int
      ): (Option[Expr], Parsel.Input[Token], Iterable[Error]) = {
        val prefixOps = levels.zipWithIndex.collect {
          case (level, index) if level.fixity == Prefix && index >= minLevel =>
            level.ops.toList
        }.flatten.toList
      ...
      def parseOperators(
        ...
        while (continue) {
          ...
          for (
            (level, levelIndex) <- levels.zipWithIndex
            if !matched && levelIndex >= minLevel
          ) {
            level.fixity match {
              case Postfix =>
                val postfixParser = or(
                  level.ops.map(
                    _.asInstanceOf[Parsel[Expr => Expr, Token, Error]]
                  )*
                )
                val (opRes, opNext, opErrs) =
                  postfixParser.parserFunc(currentInput)
              ...
              case fixity @ (InfixL | InfixR | InfixN) =>
                val infixParser = or(
                  level.ops.map(
                    _.asInstanceOf[Parsel[(Expr, Expr) => Expr, Token, Error]]
                  )*
                )
                val (opRes, opNext, opErrs) =
                  infixParser.parserFunc(currentInput)
                ...
                opRes match {
                  case Some(f) =>
                    ...
                    val (rhsOpt, rhsNext, rhsErrs) =
                      parseExprWithLevel(opNext, nextMinLevel)
  ...
  def sync[Token](
      input: Parsel.Input[Token]
  )(using spFunc: SafePointFunction[Token]): Parsel.Input[Token] = {
    var i = input
    while (
      i.current match {
  ...
  def commit[A, Token, Error](
      parser: Parsel[A, Token, Error]
  ): Parsel[A, Token, Error] = Parsel((input: Parsel.Input[Token]) => {
    val (res, next, errs) = parser.parserFunc(input)
    res match {
