...
import parser.Parsel.{Input, list, optional, or}
import parser.Token.{
  Abstract,
  Break,
  CharLiteral,
...
class TokeniserParserCombinator {
  ...
  private def parseChar(
      check: Char => Boolean
  ): Parsel[Char, Char, Diagnostic] = {
    Parsel((input: Parsel.Input[Char]) =>
      input.current match {
  ...
  private def parseString(
      str: String
  )(using fileId: Int): Parsel[String, Char, Diagnostic] = {
  ...
  private def parseSingleLineComment(using
      fileId: Int
  ): Parsel[Unit, Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false
  ...
  private def parseCharsToT[T](str: String, tokGen: SourceRange => T)(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[T, Char, Diagnostic] = {
    assert(str.nonEmpty)
    var combinator = parseChar(str(0))
    str.tail
      .foldLeft(parseChar(str.head))((combinator, char) => combinator <~ parseChar(char))
      .map((_: Char, input: Input[Char]) => {
  ...
  private val parseAlphabetOrUnderscoreOrDigit: Parsel[Char, Char, Diagnostic] = {
    Parsel((input: Input[Char]) =>
      input.current match {
        case Some(c) if c.isLetterOrDigit || c == '_' =>
          (Some(c), input.advance, List.empty)
  ...
  private def parseLeftShiftEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("<<=", Token.LeftShiftEquals(_))

  private def parseRightShiftEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(">>=", Token.RightShiftEquals(_))

  // 2-character operators
  private def parseDoubleEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("==", Token.DoubleEquals(_))

  private def parseNotEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("!=", Token.NotEquals(_))

  private def parseLessEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("<=", Token.LessEquals(_))

  private def parseGreaterEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(">=", Token.GreaterEquals(_))

  private def parseLeftShiftToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("<<", Token.LeftShift(_))

  private def parseRightShiftToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(">>", Token.RightShift(_))

  private def parsePlusEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("+=", Token.PlusEquals(_))

  private def parseMinusEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("-=", Token.MinusEquals(_))

  private def parseStarEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("*=", Token.StarEquals(_))

  private def parseSlashEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("/=", Token.SlashEquals(_))

  private def parseModuloEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("%=", Token.ModuloEquals(_))

  private def parseBitwiseAndEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("&=", Token.BitwiseAndEquals(_))

  private def parseBitwiseOrEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("|=", Token.BitwiseOrEquals(_))

  private def parseBitwiseXorEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("^=", Token.BitwiseXorEquals(_))

  private def parseIncrementToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("++", Token.Increment(_))

  private def parseDecrementToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("--", Token.Decrement(_))

  private def parseLogicalAndToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("&&", Token.LogicalAnd(_))

  private def parseLogicalOrToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("||", Token.LogicalOr(_))

  // 1-character operators and punctuation
  private def parseColonToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(":", Token.Colon(_))

  private def parseSemiColonToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(";", Token.SemiColon(_))

  private def parseCommaToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(",", Token.Comma(_))

  private def parseLeftCurlyBraceToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("{", Token.LeftCurlyBrace(_))

  private def parseRightCurlyBraceToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("}", Token.RightCurlyBrace(_))

  private def parseLeftParenToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("(", Token.LeftParen(_))

  private def parseRightParenToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(")", Token.RightParen(_))

  private def parseLeftBracketToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("[", Token.LeftBracket(_))

  private def parseRightBracketToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("]", Token.RightBracket(_))

  private def parseEqualsToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("=", Token.Equals(_))

  private def parseLessToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("<", Token.Less(_))

  private def parseGreaterToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(">", Token.Greater(_))

  private def parseNotToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("!", Token.Not(_))

  private def parsePlusToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("+", Token.Plus(_))

  private def parseMinusToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("-", Token.Minus(_))

  private def parseStarToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("*", Token.Star(_))

  private def parseSlashToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("/", Token.Slash(_))

  private def parseModuloToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("%", Token.Modulo(_))

  private def parseBitwiseAndToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("&", Token.BitwiseAnd(_))

  private def parseBitwiseOrToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("|", Token.BitwiseOr(_))

  private def parseBitwiseXorToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("^", Token.BitwiseXor(_))

  private def parseQuestionToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT("?", Token.Question(_))

  private def parseDotToken(using
      fileId: Int
  )(using SafePointFunction[Char]): Parsel[Token, Char, Diagnostic] =
    parseCharsToT(".", Token.Dot(_))

  // Literals and identifiers
  private def parseStringLiteralToken(using
      fileId: Int
  ): Parsel[Token, Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false

    (parseChar('\"') ~> list(parseChar(_ != '"')) <~ parseChar('\"')).map((chars, input) =>
      Token.StringLiteral(
        chars.mkString,
        SourceRange(fileId, input.index, 2 + chars.length)
      )
    ) <~ skipTrivia
  }

  private def parseCharLiteralToken(using
      fileId: Int
  ): Parsel[Token, Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false
  ...
  private def parseFloatLiteralToken(using
      fileId: Int
  ): Parsel[Token, Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false
    ...
    ((parseDigit ~ list(parseDigit) <~ parseChar('.')) ~ list(parseDigit)).map((tuple, input) => {
      ...
      Token.FloatLiteral(
        s"${allDigits1.mkString}.${restDigits2.mkString}".toFloat,
        SourceRange(
          fileId,
          input.index,
          1 + allDigits1.length + restDigits2.length
        )
      )
    }) <~ skipTrivia
  }

  private def parseIntegerLiteralToken(using
      fileId: Int
  ): Parsel[Token, Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false
    ...
    (parseDigit ~ list(parseDigit)).map((tuple, input) => {
      ...
      Token.IntegerLiteral(
        allDigits.mkString.toInt,
        SourceRange(fileId, input.index, allDigits.length)
      )
    }) <~ skipTrivia
  }

  private def parseIdentifierOrKeywordToken(using
      fileId: Int
  ): Parsel[Token, Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false

    (parseAlphabetOrUnderscore ~ list(parseAlphabetOrUnderscoreOrDigit)).map(
      (tuple: (Char, List[Char]), input: Input[Char]) => {
        val (c, cs) = tuple
  ...
  private def tokeniser(using
      fileId: Int
  ): Parsel[List[Token], Char, Diagnostic] = {
    given SafePointFunction[Char] = _ => false
  ...
  def tokenise(
    ...
    val (optionalTokens, _, diagnostics) = tokeniser(
      Input(string.toCharArray.toVector)
    )
